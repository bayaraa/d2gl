...
"{"
  ...
  "float f=v(i),a=v(x),e=v(m),n=v(l),b=v(d),k=min(e,min(min(f,a),min(b,n))),r=max(e,max(max(f,a),max(b,n))),g=r-k;"
  "if(g<max(1./e_t_div,r*.125))"
    "return m;"
  "vec3 F=i+x+m+l+d;"
  "float C=(f+a+n+b)*.25,A=abs(C-e),c=max(0.,A/g-.25)*(1./.75);"
  "c=min(.75,c);"
  "vec3 T=P(u,y+vec2(-s.x,-s.y)),p=P(u,y+vec2(s.x,-s.y)),t=P(u,y+vec2(-s.x,s.y)),h=P(u,y+vec2(s.xy));"
  "F+=T+p+t+h;"
  "F*=vec3(1./9.);"
  "float o=v(T),B=v(p),D=v(t),E=v(h),G=abs(.25*o+-.5*f+.25*B)+abs(.5*a+-1.*e+.5*n)+abs(.25*D+-.5*b+.25*E),H=abs(.25*o+-.5*a+.25*D)+abs(.5*f+-1.*e+.5*b)+abs(.25*B+-.5*n+.25*E);"
  "bool V=H>=G;"
  "float I=V?"
    "-s.y:"
    "-s.x;"
  ...
  "switch(v_Flags.x){"
    "case 0:"
      "FragColor=v_Flags.y==0?"
        "vec4(v(u_Texture0,v_TexCoord),1):"
        "vec4(v(u_Texture1,v_TexCoord),1);"
      "break;"
    "case 1:"
      "FragColor=v_Flags.y==0?"
        "vec4(v(u_Texture0,v_TexCoord,u_RelSize),1):"
        "vec4(v(u_Texture1,v_TexCoord,u_RelSize),1);"
      "break;"
    "case 2:"
