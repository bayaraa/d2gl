...
"{"
  "float u=max(min(v.x,v.y),min(max(v.x,v.y),v.z)),r=max(i,1.)*(u-1.+.5*y);"
  "return clamp(r+.5,0.,1.);"
"}"
"vec3 v(vec3 v,float y)"
"{"
  "float i=dot(v,vec3(.299,.587,.114));"
  "return mix(v,vec3(i),y);"
"}"
"void main()"
"{"
  "switch(v_Flags.x){"
    ...
    "case 3:"
      "if(v_Flags.w==1)"
        "{"
          "float y=v(texture(u_Textures[v_TexIds.x],vec3(v_TexCoord,v_TexIds.y)).xyz,v_Extra.x,v_Extra.y+.05),r=v(texture(u_Textures[v_TexIds.x],vec3(v_TexCoord,v_TexIds.y)).xyz,v_Extra.x,.95);"
          "FragColor=vec4(mix(v_Color2.xyz,v_Color1.xyz,r),v_Color1.w*y);"
        "}"
      "else "
         "if(v_Flags.w==2)"
          "{"
            "float y=v(texture(u_Textures[v_TexIds.x],vec3(v_TexCoord,v_TexIds.y)).xyz,v_Extra.x,v_Extra.y+.1),r=v(texture(u_Textures[v_TexIds.x],vec3(v_TexCoord,v_TexIds.y)).xyz,v_Extra.x,1.1);"
            "FragColor=vec4(mix(v_Color2.xyz,v_Color1.xyz,r),v_Color1.w*y);"
          "}"
        "else "
          "{"
            "float y=v(texture(u_Textures[v_TexIds.x],vec3(v_TexCoord,v_TexIds.y)).xyz,v_Extra.x,v_Extra.y);"
